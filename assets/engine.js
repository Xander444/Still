...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) module.exports = factory(require('./content.js'), require('./dialogue.js'));
  else root.Still = factory(root.StillContent, root.StillDialogue);
})(typeof globalThis !== 'undefined' ? globalThis : this, function (C, D) {
  'use strict';
  const MODES = ['listen', 'reflect', 'step'];
  ...
  function fresh(mode = 'reflect') {
    return { version: 2, mode: MODES.includes(mode) ? mode : 'reflect', turn: 0, topic: 'general', facts: {}, topics: [], history: [], used: [], asked: [], pending: null, safety: null, lastEmotion: null, lastInput: '', offTopic: 0, dialogue: D.fresh() };
  }
  class Engine {
    constructor(options = {}) { this.rng = options.rng || Math.random; this.state = fresh(options.mode); this.lastTrace = null; }
    setMode(mode) { if (MODES.includes(mode)) { this.state.mode = mode; if (!['grounding', 'safety'].includes(this.state.pending?.kind)) this.state.pending = null; } return this.state.mode; }
    reset(keepNotes = false) { const facts = keepNotes ? this.state.facts : {}; const prefs = keepNotes ? { ...this.state.dialogue.preferences } : null; const mode = this.state.mode; this.state = fresh(mode); this.state.facts = facts; if (prefs) this.state.dialogue.preferences = prefs; this.analysis = null; this.move = null; }
    ...
    finish(text, { rule = 'fallback', suggestions = [], pending = null, support = false, candidates = [] } = {}) {
      ...
      if (ordinary && this.state.mode !== 'step' && /\?/.test(text) && (this.state.mode === 'listen' || prefs.questions === 'few' && d.questionStreak >= 1 || prefs.questions === 'balanced' && d.questionStreak >= 2)) {
      ...
      if (ordinary && this.analysis && !support) {
        ...
        if (kind) {
          ...
          pending = null; suggestions = this.state.mode === 'listen' ? [] : ["I'd like a question", 'A small step'];
        }
      }
    ...
    welcome() { if (!this.state.history.length) this.state.history.push({ role: 'bot', text: C.welcome, suggestions: C.starters }); return this.state.history[0]; }
    ...
    restore(data) {
      ...
      if (!data || ![1, 2].includes(data.version) || typeof data !== 'object' || !Array.isArray(data.history)) return false;
    ...
    dialogueCommand(raw, t, previous) {
      ...
      for (const [pattern, key, value, reply] of preferenceRules) if (pattern.test(t)) {
        this.setPreference(key, value); if (key === 'questions' && value === 'direct') this.setMode('reflect');
      ...
      if (/^(?:that is (?:sorted|resolved|settled)(?: out)?(?: now)?|that is resolved now|we sorted that out|mark this resolved)[.!]*$/.test(t) && thread) {
        ...
        if (earlier) { d.pendingThread = earlier.id; d.lastReturnOffer = s.turn; return this.finish('We can leave that thread there. Your earlier conversation about ' + C.topics[earlier.topic].label.toLowerCase() + ' is still in the notes if you want to return to it. It is also fine to finish here.', { rule: 'thread-resolved', suggestions: ['Resume that thread', 'Wrap up'] }); }
        return this.finish('We can leave that thread there. There does not have to be another thing to work on right now.', { rule: 'thread-resolved', suggestions: ['Wrap up', 'Talk about something else'] });
      }
      ...
      if (emotionCorrection) {
        ...
        return this.finish(s.mode === 'listen' ? 'I withdraw that feeling label. You can use the words that fit for you.' : 'I withdraw that feeling label. What word would fit better?', { rule: 'repair-emotion-ask', pending: { kind: 'clarify' } });
      }
    ...
    grounding() {
      if (this.state.safety) return this.safetyFollowUp('');
      if (!this.state.dialogue.preferences.exercises) return this.finish('You asked to leave exercises out, so I have kept them off. You can enable them again in settings or say “Exercises are okay now.”', { rule: 'exercise-preference', suggestions: ['Just listen', 'Exercises are okay now'] });
    ...
    respond(input) {
      ...
      if (/\b(?:do not ask|stop asking|no more questions|no questions|just listen|only listen|need to vent|want to vent|stay with the feeling)\b/.test(t)) {
        s.mode = 'listen'; return this.finish('We can leave the questions and advice aside. Say what you need to say; it does not have to be balanced, polished, or neatly resolved.', { rule: 'mode-listen', suggestions: [] });
      }
      ...
      if (/\b(?:stop the exercise|stop exercise|end exercise|skip the exercise)\b|^(?:stop|cancel)[.!]*$/.test(t)) return this.finish('We can stop. You do not have to push through an exercise that is not helping. What would you prefer now?', { rule: 'stop', suggestions: ['Just listen', 'Talk about something else', 'Wrap up'] });
      ...
      if (forget) { this.forget(forget[1].replace(' ', '_')); return this.finish('I have removed that note. You can also erase the whole chat in settings.', { rule: 'memory-forget-one' }); }
      ...
      if (/\b(?:different topic|change (?:the )?(?:subject|topic)|talk about something else|something else entirely|new topic)\b/.test(t)) {
        s.topic = 'general'; s.lastEmotion = null; s.offTopic = 0; return this.finish('We can change direction. What would you like to talk about?', { rule: 'topic-reset', suggestions: C.starters });
      }
      ...
      if (/^(?:thank you|thanks|thank you so much|thanks a lot|that helped|this helped|that helps)[!., ]*$/.test(t)) return this.finish('You are welcome. You can leave it here if that feels like enough, or say anything that still feels unfinished.', { rule: 'thanks', suggestions: ['Wrap up', 'There is something else'] });
      ...
      const recurring = this.structuredReply(scores, true);
      ...
      if (isNo(t)) return this.finish(this.pick('no', ['Okay. We can leave that direction aside. What would feel more useful?', 'That does not fit, then. Would you rather say more in your own words?']), { rule: 'decline', suggestions: ['Just listen', 'A different topic', 'Wrap up'] });
      ...
      if (previous?.kind === 'next-step' && !strongChange) {
        if (/\b(?:neither|cannot|too hard|not doable|do not want)\b/.test(t)) return this.finish('Then that step is too big or does not fit. We can make it smaller, or leave action aside for now. What would feel less demanding?', { rule: 'step-shrink', suggestions: ['Just listen', 'A grounding pause'], pending: { kind: 'next-step' } });
      ...
      if (/\b(?:not (?:sad|anxious|angry|worried|depressed|happy)|no longer (?:sad|anxious|worried)|used to (?:be|feel) (?:sad|anxious|depressed))\b/.test(t) && !s.lastEmotion) return this.finish(s.mode === 'listen' ? 'Thank you for making that distinction. You can describe it in the words that fit for you.' : 'Thank you for making that distinction. How would you describe what you feel now?', { rule: 'emotion-negation', pending: s.mode === 'listen' ? null : { kind: 'elaborate' } });
      if (/\b(?:everyone (?:hates|dislikes|thinks)|nobody (?:cares|likes|loves)|no one (?:cares|likes|loves)|always (?:fail|mess)|never (?:get anything right|succeed)|i am (?:worthless|a failure|useless|unlovable|a loser))\b/.test(t)) {
        return this.finish('That sounds painful to believe about yourself. I can take the feeling seriously without agreeing that it tells the whole story.' + (s.mode === 'listen' ? '\n\nYou can say more about what has brought you to this point.' : '\n\nWhat happened most recently that made that conclusion feel true?'), { rule: 'validate-not-verdict', pending: s.mode === 'listen' ? null : { kind: 'elaborate' } });
      }
      if (/\b(?:tell me (?:everything|it) will be (?:okay|fine)|promise (?:everything|it) will|reassure me|am i going to fail)\b/.test(t)) return this.finish('I cannot promise an outcome I do not know. What I can say is that a difficult outcome would not define your whole worth, and you do not have to work through the uncertainty alone.' + (s.mode === 'listen' ? '' : '\n\nWhich part of not knowing is hardest right now?'), { rule: 'honest-reassurance', pending: s.mode === 'listen' ? null : { kind: 'elaborate' } });
      ...
      if (thirdPerson) return this.finish('It sounds like you are noticing what ' + (s.facts.person?.value || 'someone close to you').replace(/^my /i, 'your ') + ' is going through.' + (s.mode === 'listen' ? '\n\nThere is room for your experience of it, too.' : '\n\nHow is it affecting you?'), { rule: 'third-person', pending: s.mode === 'listen' ? null : { kind: 'elaborate' } });
      const structured = this.structuredReply(scores);
      if (structured) return structured;
      if (/\bi (?:failed|did not pass|got a bad grade|did badly)\b/.test(t)) return this.finish('A disappointing result can hurt. It does not settle what you are capable of.' + (s.mode === 'listen' ? '\n\nYou do not have to turn the disappointment into a plan right away.' : '\n\nWhat feels hardest: the result itself, what comes next, or what you are telling yourself about it?'), { rule: 'school-setback', suggestions: s.mode === 'listen' ? [] : ['The result itself', 'What comes next', 'What I tell myself'], pending: s.mode === 'listen' ? null : { kind: 'question', id: 'school-pressure' } });
      if (topic === 'joy' && /\b(?:something good|good news|actually pretty good)\b/.test(t)) return this.finish('There is room for good things here, too. Tell me what happened.' + (s.mode === 'listen' ? '' : ' What feels good about it?'), { rule: 'joy-opening', pending: { kind: 'elaborate' } });
      const isSchoolEvent = /\b(?:exam|test|quiz|midterm)\b/.test(t) && !s.facts.subject && topic !== 'joy';
      if (isSchoolEvent && s.mode !== 'listen') {
      ...
      if (s.mode === 'listen') {
        ...
        return this.finish([emotionLine, listening].filter(Boolean).join(' '), { rule: 'listen', suggestions: s.turn % 3 === 0 ? ["I'd like a question", 'A small step', 'Wrap up'] : [], candidates: scores });
      }
      ...
      if (!question) return this.finish([lead, 'We have explored a few sides of this. Would it help to stay with what you feel, choose a small step, or leave it here for now?'].filter(Boolean).join('\n\n'), { rule: 'topic-complete', suggestions: ['Just listen', 'A small step', 'Wrap up'], candidates: scores });
